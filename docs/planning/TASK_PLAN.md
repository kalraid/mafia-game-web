...
> **문서 버전**: v2.6
> **최초 작성일**: 2026-03-18
> **최종 업데이트**: 2026-04-06 (GAP-05~07 반영: 문서 플로우·RAG 로그·I-19 병렬 AI 턴 보류)
> **기준 커밋**: `e180eb6`
...
| # | 내용 |
|---|------|
| **I-19** | **병렬 AI 턴** (`asyncio.gather` 등으로 `run_*_round` 동시 실행): **착수 보류** (GAP-07, 2026-04-06). 낮 채팅 **발화 순서**·`GameState` **동시 갱신**·WebSocket **이벤트 순서**가 얽히면 재현·디버깅 비용이 커짐. `PROPOSAL.md` §5 확장 과제로 유지하고, 필요 시 전용 설계 티켓(락/큐/페이즈 내 부분 병렬)으로 분리. |
