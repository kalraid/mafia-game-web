...
| NIGHT_MAFIA 라운드 | `run_night_mafia_round()` + mafia_secret 채널 | `af1a977` |
| Phase 가드 | `run()`에서 Phase별 허용 행동 필터링 | `af1a977` |
| CI 제어 | `MAFIA_USE_LLM=0` 플래그로 LLM 비활성화 | `af1a977` |
...
**현황**: `graph.py`의 `run_night_mafia_round()`에서 `mafia_secret` 채널로 전송하나, `websocket/manager.py`의 `broadcast()`가 채널 구분 없이 모든 클라이언트에게 전송함. 시민도 마피아 채팅을 볼 수 있는 상태.
...
async def broadcast_to_channel(
    ...
    """channel이 mafia_secret이면 allowed_player_ids에게만 전송."""
    ...

# runner.py 또는 engine에서
# NIGHT_MAFIA phase 진입 시 마피아 player_id 목록을 WS 매니저에 전달
```
...
**현황**: `graph.py`에 `thread_id` config만 있고 실제 Redis Checkpointer 없음. Agent가 라운드 간 이전 관찰을 기억하지 못함.

**목표**: `thread_id = f"{game_id}_{agent_id}"` 기준으로 Agent별 상태 영속화.
...
# AgentGraph 컴파일 시 checkpointer 주입
# ⚠️ 현재 _DayChatState에 PlayerAgent 등 비직렬화 객체가 있어
#    상태를 직렬화 가능한 형태로 변경 먼저 필요
